...
class LogisticRegressionL2Norm:
    ...
    a line between two classes. This algorithm uses L2 norm to minimizes coefficients.

    """

    @staticmethod
    def gradient_ascent(feature_matrix, label, initial_coefficients, step_size, max_iter, l2_penalty):
        """Gradient ascent algorithm with L2 Norm for Logistic Regression.

        The gradient ascent algorithm: w^(t+1) <= w^(t) + n(Σ^N_i=1(h_j(X_i))(1[y=+1]-P(y=1|x_i,w))-2*λ*w(t)).
        Where:
